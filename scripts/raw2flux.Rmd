...
files.sources = list.files(paste0(repo_root,"/functions"), full.names = T)
...
my_incub <- mydata[as.numeric(mydata$POSIX.time)> myauxfile$start.time &
                     as.numeric(mydata$POSIX.time)< myauxfile$start.time+myauxfile$duration,]
my_incub <- my_incub[!is.na(my_incub$CO2dry_ppm),]
...
myfluxterm <- flux.term(myauxfile$Vtot[i], myauxfile$Pcham[i], myauxfile$Area[i],
                        myauxfile$Tcham[i], first(H2O_mol))
...
CH4_res_meth1$total_estimated[which(CH4_res_meth1$UniqueID==myauxfile$UniqueID[i])] <- CH4_flux_total
CH4_res_meth1$ebullition[which(CH4_res_meth1$UniqueID==myauxfile$UniqueID[i])] <- CH4_flux_ebull
CH4_res_meth1$diffusion[which(CH4_res_meth1$UniqueID==myauxfile$UniqueID[i])] <- CH4_flux_diff
...
for (subsite in subsites){
  ...
  if(dir.exists(path2data)){
    ...
    for (i in seq_along(myauxfile)){
      my_incub <- mydata[as.numeric(mydata$POSIX.time)> myauxfile$start.time[i] &
                           as.numeric(mydata$POSIX.time)< myauxfile$start.time[i]+myauxfile$duration[i],]
      my_incub <- my_incub[!is.na(my_incub$CO2dry_ppm),]
      # calling dedicated function
      df_ebull <- separate_ebullition_from_diffusion(my_incub = my_incub)
      # computing fluxes
